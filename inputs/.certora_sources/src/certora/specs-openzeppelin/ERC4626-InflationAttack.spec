...
//Had to change _ERC20 to _assetsVault as of import that already declares assetsVault.
using ERC20Mock as assetsVault;

methods{
    function assetsVault.allowance(address,address) external returns uint256 envfree;
    function assetsVault.balanceOf(address) external returns uint256 envfree;
    function assetsVault.decimals() external returns uint8 envfree;
    function assetsVault.totalSupply() external returns uint256 envfree;
...
*/
rule preventInflationAttack(uint256 assets, address deposit_receiver, address redeem_receiver, address redeem_owner) {
    env e;
...
rule complexVersionOfInflationAttack(address attacker, address victim, address deposit1_receiver, address deposit2_victim_receiver,address redeem_receiver,address redeem_owner ){
    ...
    require(attacker != currentContract);
    require(attacker != assetsVault);
    require(attacker != 0);
    require(victim != currentContract);
    require(victim != assetsVault);
    require(victim != 0);
    ...
    require(assetsVault.balanceOf(currentContract) + assetsVault.balanceOf(attacker) + assetsVault.balanceOf(victim) + assetsVault.balanceOf(deposit1_receiver) +assetsVault.balanceOf(deposit2_victim_receiver) +assetsVault.balanceOf(redeem_receiver) + assetsVault.balanceOf(redeem_owner) <= to_mathint(assetsVault.totalSupply()));
    ...
    */
    ...
    require(assetsVault.balanceOf(attacker) >= amountDirectTransferToUnderlying);

    /**
    * Step 2: the attacker also transfers 1 * 1e18 weiWETH, making the totalAssets() WETH balance of the vault become 1e18 + 1 wei
    */
    assetsVault.transferFrom(e2, attacker, currentContract, amountDirectTransferToUnderlying);
    require(assetsVault.balanceOf(currentContract) > 0);
